...
type userR struct {
	TodoLists TodoListSlice `boil:"TodoLists" json:"TodoLists" toml:"TodoLists" yaml:"TodoLists"`
}
...
func (r *userR) GetTodoLists() TodoListSlice {
	if r == nil {
		return nil
	}
	return r.TodoLists
...
// TodoLists retrieves all the todo_list's TodoLists with an executor.
func (o *User) TodoLists(mods ...qm.QueryMod) todoListQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"todo_list\".\"user_id\"=?", o.ID),
	)

	return TodoLists(queryMods...)
}

// LoadTodoLists allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadTodoLists(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`todo_list`),
		qm.WhereIn(`todo_list.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load todo_list")
	}

	var resultSlice []*TodoList
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice todo_list")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on todo_list")
	}
	...
	if singular {
		object.R.TodoLists = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.TodoLists = append(local.R.TodoLists, foreign)
				if foreign.R == nil {
					foreign.R = &todoListR{}
				}
...
// AddTodoLists adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.TodoLists.
// Sets related.R.User appropriately.
func (o *User) AddTodoLists(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TodoList) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.TodoLists = append(o.R.TodoLists, related...)
	}
