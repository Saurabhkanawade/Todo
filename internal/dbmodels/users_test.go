...
func testUserToManyTodoLists(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, todoListDBTypes, false, todoListColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, todoListDBTypes, false, todoListColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.TodoLists().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadTodoLists(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TodoLists); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TodoLists = nil
	if err = a.L.LoadTodoLists(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TodoLists); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpTodoLists(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e TodoList

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*TodoList{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, todoListDBTypes, false, strmangle.SetComplement(todoListPrimaryKeyColumns, todoListColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*TodoList{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddTodoLists(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.TodoLists[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TodoLists[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TodoLists().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Name`: `character varying`, `Email`: `character varying`, `PasswordHash`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
