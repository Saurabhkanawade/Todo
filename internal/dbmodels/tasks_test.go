...
func testTaskToManyTaskTags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TaskSlice{&a}
	if err = a.L.LoadTaskTags(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTaskToManyAddOpTaskTags(t *testing.T) {
	var err error
...
func testTaskToOneTodoListUsingTodoList(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local Task
	var foreign TodoList

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, taskDBTypes, false, taskColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Task struct: %s", err)
	}
	if err := randomize.Struct(seed, &foreign, todoListDBTypes, false, todoListColumnsWithDefault...); err != nil {
	...
	local.TodoListID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.TodoList().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	AddTodoListHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *TodoList) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := TaskSlice{&local}
	if err = local.L.LoadTodoList(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.TodoList == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.TodoList = nil
	if err = local.L.LoadTodoList(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.TodoList == nil {
		t.Error("struct should have been eager loaded")
...
func testTaskToOneSetOpTodoListUsingTodoList(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, todoListDBTypes, false, strmangle.SetComplement(todoListPrimaryKeyColumns, todoListColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, todoListDBTypes, false, strmangle.SetComplement(todoListPrimaryKeyColumns, todoListColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*TodoList{&b, &c} {
		err = a.SetTodoList(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.TodoListID != x.ID {
			t.Error("foreign key was wrong value", a.TodoListID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.TodoListID))
		reflect.Indirect(reflect.ValueOf(&a.TodoListID)).Set(zero)
		...
		if a.TodoListID != x.ID {
			t.Error("foreign key was wrong value", a.TodoListID, x.ID)
		}
	}
}
...
var (
	taskDBTypes = map[string]string{`ID`: `uuid`, `TodoListID`: `uuid`, `Title`: `character varying`, `Description`: `text`, `DueDate`: `timestamp without time zone`, `Ppriority`: `enum.task_priority('low','medium','high')`, `Status`: `enum.task_status('pending','in progress','completed')`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
